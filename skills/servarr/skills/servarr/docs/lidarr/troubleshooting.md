<!-- Source: https://wiki.servarr.com/lidarr/troubleshooting -->

* * *

Lidarr Troubleshooting

Common issues, error codes, and solutions for troubleshooting Lidarr installation, configuration, and operational problems
...
# [¶](https://wiki.servarr.com/lidarr/troubleshooting\#asking-for-help) Asking for Help
...
1. [Turn Logging up to Trace (Settings => General => Log Level or Edit The Config File)](https://wiki.servarr.com/lidarr/troubleshooting#tracedebug-logs)
2. [Clear Logs (System => Logs => Clear Logs or Delete all the Logs in the Log Folder)](https://wiki.servarr.com/lidarr/troubleshooting#clearing-logs)
3. Reproduce the Issue (Redo what is breaking things)
4. [Open the trace log file (Lidarr.trace.txt) via the UI or the log file](https://wiki.servarr.com/lidarr/troubleshooting#standard-logs-location) on the filesystem and find the relevant context
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#standard-logs-location) Standard Logs Location

The log files are located in Lidarr's [Appdata Directory](https://wiki.servarr.com/lidarr/appdata-directory), inside the logs/ folder. You can also access the log files from the UI at System => Logs => Files.
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#update-logs-location) Update Logs Location

The update log files are located in Lidarr's [Appdata Directory](https://wiki.servarr.com/lidarr/appdata-directory), inside the UpdateLogs/ folder.

## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#sharing-logs) Sharing Logs
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#tracedebug-logs) Trace/Debug Logs
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#clearing-logs) Clearing Logs
...
# [¶](https://wiki.servarr.com/lidarr/troubleshooting\#multiple-log-files) Multiple Log Files
...
# [¶](https://wiki.servarr.com/lidarr/troubleshooting\#recovering-from-a-failed-update) Recovering from a Failed Update
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#determine-the-issue) Determine the issue

- The best place to look when the application will not start after an update is to review the [update logs](https://wiki.servarr.com/lidarr/troubleshooting#update-logs-location) and see if the update completed successfully. If those do not have an issue then the next step is to look at your regular application log files, before trying to start again, use [Logging](https://wiki.servarr.com/lidarr/settings#logging) and [Log Files](https://wiki.servarr.com/lidarr/system#log-files) to find them and increase the log level.
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#migration-issue) Migration Issue
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#permission-issue) Permission Issue
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#resolving-the-issue) Resolving the issue
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#permissions-issues) Permissions Issues
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#manually-upgrading) Manually upgrading
...
# [¶](https://wiki.servarr.com/lidarr/troubleshooting\#downloads-and-importing) Downloads and Importing

Downloading and importing is where _most_ people experience issues. From a high level perspective, Lidarr needs to be able to communicate with your download client and have access to the files it downloads. There is a large variety of supported download clients and an even _bigger_ variety of setups. This means that while there are some _common_ setups, there isn’t one _right_ setup and everyone’s setup can be a little different.

> **The first step is to turn logging up to Trace, see [Logging and Log Files](https://wiki.servarr.com/lidarr/troubleshooting#logging-and-log-files) for details on adjusting logging and searching logs. You’ll then reproduce the issue and use the trace level logs from that time frame to examine the issue.** If someone is helping you, put context from before/after in a [pastebin](https://0bin.net/), [Gist](https://gist.com/), or similar site to show them. It doesn’t need to be the whole file and it shouldn’t _just_ be the error. You should also reproduce the issue while tasks that spam the log file aren’t running.

When you reach out for help, be sure to read [asking for help](https://wiki.servarr.com/lidarr/troubleshooting#asking-for-help) so that you can provide us with the details we’ll need.

## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#testing-the-download-client) Testing the Download Client
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#testing-a-download) Testing a Download
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#testing-an-import) Testing an Import
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#common-problems) Common Problems

Below are some common problems.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#download-clients-webui-is-not-enabled) Download Client's WebUI is not enabled
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#ssl-in-use-and-incorrectly-configured) SSL in use and incorrectly configured

Ensure SSL encryption is not turned on if you're using both your instance and your download client on a local network. See [the SSL FAQ entry](https://wiki.servarr.com/lidarr/faq#invalid-certificate-and-other-HTTPS-or-SSL-issues) for more information.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#cant-see-share-on-windows) Can’t see share on Windows

The default user for a Windows service is `LocalService` which typically doesn’t have access to your shares. Edit the service and set it up to run as your own user, see the FAQ entry [why can’t see my files on a remote server](https://wiki.servarr.com/lidarr/faq#why-cant-see-my-files-on-a-remote-server) for details.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#mapped-network-drives-are-not-reliable) Mapped network drives are not reliable
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#docker-and-user-group-ownership-permissions-and-paths) Docker and user, group, ownership, permissions and paths
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#remote-path-mapping) Remote Path Mapping
...
- [Settings => Download Clients => Remote Path Mappings](https://wiki.servarr.com/lidarr/settings#remote-path-mappings)
...
#### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#remote-mount-or-remote-sync-syncthing) Remote Mount or Remote Sync (Syncthing)
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#permissions-on-the-library-folder) Permissions on the Library Folder
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#permissions-on-the-downloads-folder) Permissions on the Downloads Folder
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#download-folder-and-library-folder-not-different-folders) Download folder and library folder not different folders
...
- [**This will also cause a healthcheck in System as well**](https://wiki.servarr.com/lidarr/system#downloading-into-root-folder)
...
- Configured Root Folders (aka Library folders) can be found in [Settings => Media Management => Root Folders](https://wiki.servarr.com/lidarr/settings/#root-folders)
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#incorrect-category) Incorrect category
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#packed-torrents) Packed torrents
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#repeated-downloads) Repeated downloads

There are a few causes of repeated downloads, but one is related to the Indexer restriction in Release Profiles. Because the indexer _isn’t_ stored with the data, any preferred word scores are _zero_ for media in your library, _but_ during “RSS” and search, they’ll be applied. This gets you into a loop where you download the items again and again because it looks like an upgrade, then isn’t, then shows up again and looks like an upgrade, then isn’t. Don’t restrict your release profile to an indexer.
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#usenet-download-misses-import) Usenet download misses import

Lidarr only looks at the 60 most recent downloads in SABnzbd and NZBGet, so if you _keep_ your history this means that during large queues with import issues, downloads can be silently missed and not imported. The best way to avoid that is to keep your history clear, so that any items that still appear need investigating. You can achieve this by enabling Remove under Completed and Failed Download Handler. In NZBGet, this will move items to the _hidden_ history which is great. Unfortunately, SABnzbd does not have a similar feature. The best you can achieve there is to use the nzb backup folder.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#download-client-clearing-items) Download client clearing items
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#download-cannot-be-matched-to-a-library-item) Download cannot be matched to a library item
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#the-underlying-connection-was-closed-an-unexpected-error-occurred-on-a-send) The underlying connection was closed: An unexpected error occurred on a send
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#the-request-timed-out) The request timed out
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#problem-not-listed) Problem Not Listed
...
# [¶](https://wiki.servarr.com/lidarr/troubleshooting\#searches-indexers-and-trackers) Searches Indexers and Trackers
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#turn-logging-up-to-trace) Turn logging up to trace

> **The first step is to turn logging up to Trace, see [Logging and Log Files](https://wiki.servarr.com/lidarr/troubleshooting#logging-and-log-files) for details on adjusting logging and searching logs. You’ll then reproduce the issue and use the trace level logs from that time frame to examine the issue.** If someone is helping you, put context from before/after in a [pastebin](https://0bin.net/), [Gist](https://gist.com/), or similar site to show them. It doesn’t need to be the whole file and it shouldn’t _just_ be the error. You should also reproduce the issue while tasks that spam the log file aren’t running.

## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#testing-an-indexer-or-tracker) Testing an Indexer or Tracker
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#testing-a-search) Testing a Search
...
![searches-indexers-and-trackers1.png](https://wiki.servarr.com/assets/lidarr/searches-indexers-and-trackers1.png)

![searches-indexers-and-trackers2.png](https://wiki.servarr.com/assets/lidarr/searches-indexers-and-trackers2.png)
...
## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#common-problems-1) Common Problems

Below are some common problems.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#unable-to-load-search-results) Unable to Load Search Results
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#media-is-unmonitored) Media is Unmonitored

The song(s) is(are) not monitored.

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#tracker-needs-rawsearch-caps) Tracker needs RawSearch Caps
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#wrong-categories) Wrong categories
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#wrong-results) Wrong results
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#query-successful-no-results-returned) Query Successful - No Results returned
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#missing-results) Missing Results
...
- [Categories are incorrect - See Above](https://wiki.servarr.com/lidarr/troubleshooting#wrong-categories)
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#certificate-validation) Certificate validation
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#hitting-rate-limits) Hitting rate limits
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#ip-ban) IP Ban
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#using-the-jackett-all-endpoint) Using the Jackett /all endpoint
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#using-nzbhydra2-as-a-single-entry) Using NZBHydra2 as a single entry
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#problem-not-listed-1) Problem Not Listed

You can also review some common permissions and networking troubleshooting commands [in our guide](https://wiki.servarr.com/permissions-and-networking). Otherwise please discuss with the support team on discord. If this is something that may be a common problem, please suggest adding it to the wiki.

## [¶](https://wiki.servarr.com/lidarr/troubleshooting\#errors) Errors

These are some of the common errors you may see when adding an indexer

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#the-underlying-connection-was-closed-an-unexpected-error-occurred-on-a-send-1) The underlying connection was closed: An unexpected error occurred on a send

This is caused by the indexer using a SSL protocol not supported by the current .NET Version found in [Lidarr => System => Status](https://wiki.servarr.com/lidarr/system#status).

### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#the-request-timed-out-1) The request timed out
...
### [¶](https://wiki.servarr.com/lidarr/troubleshooting\#problem-not-listed-2) Problem Not Listed
