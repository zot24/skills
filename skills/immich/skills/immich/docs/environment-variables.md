...
| `IMMICH_HELMET_FILE`                | Path to a json file with <a href="https://www.npmjs.com/package/helmet" target="_blank" rel="noopener noreferrer">helmet</a> options. Set to `false` to disable. Set to `true` to use `server/helmet.json`<sup>\*3</sup>. |           `false`            | server                   | api                |
...
\*3: The <a href="https://helmetjs.github.io/#content-security-policy" target="_blank" rel="noopener noreferrer">default configuration</a> sets `upgrade-insecure-requests`, which tells the browser to upgrade all requests to HTTPS. This breaks on HTTP-only deployments. If you cannot use HTTPS, you should use a custom helmet config file with `"upgrade-insecure-requests": null`.
