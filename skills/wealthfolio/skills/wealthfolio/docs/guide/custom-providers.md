...
For a separate cryptocurrency example, CoinGecko may expect a coin ID such as `bitcoin`, rather than the ticker used by another provider.
...
After switching back to automatic pricing, check the price history and source. Provider quotes may differ from your manually entered values.
