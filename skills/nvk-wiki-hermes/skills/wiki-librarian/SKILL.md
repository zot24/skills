...
name: wiki-librarian
description: >-
  Keep the active wiki article layer in check: staleness, quality, accuracy, and coherence. Focused maintenance tool; broader trust audits belong to /wiki:audit. Use when the user runs /wiki-librarian or /wiki:librarian. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mv:*), Bash(mkdir:*), WebFetch, WebSearch, Agent
---

# wiki-librarian — nvk v0.23.0 `/wiki:librarian`
...
- Hermes slash: `/wiki-librarian` (hyphen). Claude: `/wiki:librarian` (colon). Hermes cannot use colons in slashes.
...
- `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/librarian.md` (in-skill copy: `references/librarian.md`; tag: https://raw.githubusercontent.com/nvk/llm-wiki/v0.23.0/plugins/llm-wiki-opencode/skills/wiki-manager/references/librarian.md)

Official command body follows. `$ARGUMENTS` is the text after `/wiki-librarian`.
...
Read the librarian reference at `references/librarian.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/librarian.md`). Then execute the requested subcommand.
