...
name: wiki-assess
description: >-
  Assess a local repo against the active wiki's research body and the broader market. Gap analysis, opportunities, competitive landscape. Use when the user runs /wiki-assess or /wiki:assess. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(find:*), Bash(head:*), WebFetch, WebSearch, Agent
---

# wiki-assess — nvk v0.23.0 `/wiki:assess`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/assess.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-assess` (hyphen). Claude: `/wiki:assess` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-assess`.
