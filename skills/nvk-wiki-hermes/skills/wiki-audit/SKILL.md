...
name: wiki-audit
description: >-
  Truth-seeking umbrella audit for llm-wiki. Combines active wiki maintenance, output drift checks, provenance review, and fresh research when trust is in doubt. Use when the user runs /wiki-audit or /wiki:audit. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mkdir:*), WebFetch, WebSearch, Agent
---

# wiki-audit — nvk v0.23.0 `/wiki:audit`
...
- Hermes slash: `/wiki-audit` (hyphen). Claude: `/wiki:audit` (colon). Hermes cannot use colons in slashes.
...
- `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/audit.md` (in-skill copy: `references/audit.md`; tag: https://raw.githubusercontent.com/nvk/llm-wiki/v0.23.0/plugins/llm-wiki-opencode/skills/wiki-manager/references/audit.md)

Official command body follows. `$ARGUMENTS` is the text after `/wiki-audit`.
...
Read the audit reference at `references/audit.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/audit.md`). If you need a fresh wiki-only pass, also read `commands/librarian.md` and reuse its scan protocol rather than inventing a parallel one.
