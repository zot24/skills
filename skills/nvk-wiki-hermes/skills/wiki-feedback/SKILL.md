...
name: wiki-feedback
description: >-
  Review, capture, and promote curated user-feedback candidates detected from session hooks. Use when the user runs /wiki-feedback or /wiki:feedback. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(mkdir:*), Bash(python3:*), Bash(scripts/llm-wiki-session:*)
---

# wiki-feedback — nvk v0.23.0 `/wiki:feedback`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/feedback.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-feedback` (hyphen). Claude: `/wiki:feedback` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-feedback`.
