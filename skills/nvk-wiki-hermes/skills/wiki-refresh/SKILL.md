...
name: wiki-refresh
description: >-
  Check if active wiki articles are still current. Re-fetches sources, detects changes, and offers to update stale articles. Use when the user runs /wiki-refresh or /wiki:refresh. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), WebFetch, WebSearch, Agent
---

# wiki-refresh — nvk v0.23.0 `/wiki:refresh`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/refresh.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-refresh` (hyphen). Claude: `/wiki:refresh` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-refresh`.
