...
name: wiki-ingest
description: >-
  Ingest source material into an active wiki. Accepts URLs, file paths, PDFs, freeform text, or processes the inbox. Supports tweets via Grok MCP. Use when the user runs /wiki-ingest or /wiki:ingest. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
...
# wiki-ingest — nvk v0.23.0 `/wiki:ingest`
...
- Hermes slash: `/wiki-ingest` (hyphen). Claude: `/wiki:ingest` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-ingest`.
...
Read the ingestion protocol at `references/ingestion.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/ingestion.md`) and the structure spec at `references/wiki-structure.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/wiki-structure.md`). Then ingest source material.
