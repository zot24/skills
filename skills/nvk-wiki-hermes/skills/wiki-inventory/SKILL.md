...
name: wiki-inventory
description: >-
  Track wiki-adjacent things the user cares about: items, Ideas, ingest candidates, entities, corpora, open questions, tasks, and other durable inventory records. Use when the user runs /wiki-inventory or /wiki:inventory. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mkdir:*)
---

# wiki-inventory — nvk v0.23.0 `/wiki:inventory`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/inventory.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-inventory` (hyphen). Claude: `/wiki:inventory` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-inventory`.
...
After resolving the wiki, read the inventory reference at `references/inventory.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/inventory.md`), then run the requested subcommand.
