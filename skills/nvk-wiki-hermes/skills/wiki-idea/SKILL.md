...
name: wiki-idea
description: >-
  Capture, research, shape, catalog, and explicitly promote Ideas into Projects through natural language. Use when the user runs /wiki-idea or /wiki:idea. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, WebFetch, WebSearch, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mkdir:*)
---

# wiki-idea — nvk v0.23.0 `/wiki:idea`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/idea.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-idea` (hyphen). Claude: `/wiki:idea` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-idea`.

---
## Your task

Manage Ideas between compiled Concepts and committed Projects. Read
`references/ideas.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/ideas.md`); load inventory, research, or Project
