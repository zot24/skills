...
name: wiki-retract
description: >-
  Remove user-selected data or a source from wiki-controlled local files, including archives and session context. Dry-run first, apply explicitly, then verify. Use when the user runs /wiki-retract or /wiki:retract. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(rm:*), Bash(grep:*), Bash(scripts/llm-wiki:*)
---

# wiki-retract — nvk v0.23.0 `/wiki:retract`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/retract.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-retract` (hyphen). Claude: `/wiki:retract` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-retract`.
