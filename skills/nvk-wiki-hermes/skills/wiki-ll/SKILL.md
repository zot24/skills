...
name: wiki-ll
description: >-
  Extract lessons learned from the current session. Scans conversation for error→fix patterns, user corrections, and discoveries, then saves structured lessons to the wiki. Use when the user runs /wiki-ll or /wiki:ll. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mkdir:*)
---

# wiki-ll — nvk v0.23.0 `/wiki:ll`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/ll.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-ll` (hyphen). Claude: `/wiki:ll` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-ll`.
