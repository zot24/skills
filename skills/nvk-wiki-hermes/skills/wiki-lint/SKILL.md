...
name: wiki-lint
description: >-
  Run health checks on the wiki. Find broken links, missing indexes, stale content, archive registry drift, inconsistencies, and suggest improvements. Use when the user runs /wiki-lint or /wiki:lint. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mv:*), Bash(mkdir:*), WebSearch
---

# wiki-lint — nvk v0.23.0 `/wiki:lint`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/lint.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-lint` (hyphen). Claude: `/wiki:lint` (colon). Hermes cannot use colons in slashes.
...
- `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/linting.md` (in-skill copy: `references/linting.md`; tag: https://raw.githubusercontent.com/nvk/llm-wiki/v0.23.0/plugins/llm-wiki-opencode/skills/wiki-manager/references/linting.md)

Official command body follows. `$ARGUMENTS` is the text after `/wiki-lint`.
...
Read the linting rules at `references/linting.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/linting.md`). Then run health checks on the wiki.
