...
name: wiki-project
description: >-
  Manage projects inside a topic wiki. Projects are folders under output/projects/ that group related outputs (playbooks, images, code, data) with a goal captured in WHY.md. Use when the user runs /wiki-project or /wiki:project. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(mkdir:*), Bash(mv:*), Bash(date:*), Bash(basename:*), Bash(find:*), Bash(wc:*)
---

# wiki-project — nvk v0.23.0 `/wiki:project`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/project.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-project` (hyphen). Claude: `/wiki:project` (colon). Hermes cannot use colons in slashes.
...
- `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/ideas.md` (in-skill copy: `references/ideas.md`; tag: https://raw.githubusercontent.com/nvk/llm-wiki/v0.23.0/plugins/llm-wiki-opencode/skills/wiki-manager/references/ideas.md)

Official command body follows. `$ARGUMENTS` is the text after `/wiki-project`.
...
Read the projects architecture at `references/projects.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/projects.md`) for the full rationale — particularly *why* `WHY.md` is the only required file (it holds the precious, non-derivable rationale) and *why* everything else is derived from filesystem state.
...
`references/ideas.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/ideas.md`). Promotion freezes `BRIEF.md` and
