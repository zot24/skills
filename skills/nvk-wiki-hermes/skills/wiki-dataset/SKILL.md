...
name: wiki-dataset
description: >-
  Manage dataset manifests for data that is too large or unsuitable to store directly in the wiki. The wiki becomes the index/interface; the data stays external. Use when the user runs /wiki-dataset or /wiki:dataset. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mkdir:*), Bash(du:*), Bash(stat:*), Bash(head:*), Bash(file:*)
---

# wiki-dataset — nvk v0.23.0 `/wiki:dataset`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/dataset.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-dataset` (hyphen). Claude: `/wiki:dataset` (colon). Hermes cannot use colons in slashes.
...
- `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/datasets.md` (in-skill copy: `references/datasets.md`; tag: https://raw.githubusercontent.com/nvk/llm-wiki/v0.23.0/plugins/llm-wiki-opencode/skills/wiki-manager/references/datasets.md)

Official command body follows. `$ARGUMENTS` is the text after `/wiki-dataset`.
...
After resolving the wiki, read the dataset reference at `references/datasets.md` (vendored v0.23.0; pin `~/.claude/plugins/marketplaces/llm-wiki/plugins/llm-wiki-opencode/skills/wiki-manager/references/datasets.md`), then run the requested subcommand.
