...
name: wiki-output
description: >-
  Generate output artifacts from active wiki content — summaries, reports, study guides, slide outlines, timelines, glossaries, comparisons. Outputs are filed back into the wiki. Use when the user runs /wiki-output or /wiki:output. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(date:*), Bash(python3:*)
---

# wiki-output — nvk v0.23.0 `/wiki:output`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/output.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-output` (hyphen). Claude: `/wiki:output` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-output`.
