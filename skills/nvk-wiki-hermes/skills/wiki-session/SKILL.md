...
name: wiki-session
description: >-
  Capture, list, rehydrate, and promote llm-wiki session context. Supports automated hook capture into HUB/.sessions plus explicit promotion into topic raw notes. Use when the user runs /wiki-session or /wiki:session. Official nvk/llm-wiki v0.23.0 command body. Never use bundled Karpathy llm-wiki against this hub.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(mkdir:*), Bash(python3:*), Bash(scripts/llm-wiki-session:*)
---

# wiki-session — nvk v0.23.0 `/wiki:session`

Vendored **verbatim** from nvk/llm-wiki **v0.23.0** (`d02cbcb`)
`claude-plugin/commands/session.md`.
Do **not** invent compile, ingest, lint, or query protocols.

- Hermes slash: `/wiki-session` (hyphen). Claude: `/wiki:session` (colon). Hermes cannot use colons in slashes.
...
Official command body follows. `$ARGUMENTS` is the text after `/wiki-session`.
